...
# @app.route("/bar_chart")
# def bar():
#         # accuracy1 = accuracy_score(y_test, predictionstest)
#         # precision1 = precision_score(y_test, predictionstest, average='macro')
#         # recall1 = recall_score(y_test, predictionstest, average='macro')
#         line_chart = pygal.Bar()
#         # accuracyscore1, accuracyscore2, accuracyscore3, accuracyscore4
#         line_chart.title = 'ACCURACY, PRECISION AND RECALL SCORES'
#         # line_chart.add('ACCURACY',accuracy1 )
#         # line_chart.add('PRECISION',precision1 )
#         # line_chart.add('RECALL', recall1 )
#
#         line_chart.add('ACCURACY',accuracyscore1 )
#         line_chart.add('PRECISION',accuracyscore2 )
#         line_chart.add('RECALL', accuracyscore3 )
#         line_chart.add('RECALL1', accuracyscore4 )
#
...
def uploaddataset_csv_submitted():
    if request.method == "POST":
        ...
        file = "C:/Users/DELL/Downloads/" + result
        print(file)
...
def traintestdataset_submitted():
    if request.method == "POST":
      ...
      #  df1 = pd.read_csv(filepath)
        ...
        # X= df1.drop(['PERID', 'Criminal'], axis=1)
        # y= df1['Criminal']
        X= df1.drop([ 'Criminal'], axis=1)
        y= df1['Criminal']
        # print(y)
        ...
        return render_template('traintestdataset.html', msg='sucess',data1=X_test1.head().to_html(), data=X_train1.head().to_html(),X_trainlenvalue=X_trainlen, y_testlenval=y_testlen)
    return render_template('traintestdataset.html')
...
#        list1.extend([Open]) #Tunnover
        ...
        if (listLength==38):
            ...
            model = linear_model.LinearRegression()
            model.fit(X_trains, y_trains.values.ravel())
            # y_pred = model.predict(X_tests)
            ...
            print(pre)
            threshold=0.5
            binary_predictions = ["danger" if pre >= threshold else "normal"]
            ...
            return render_template('prediction.html', msg='predictsucess',msg1=outmsg,predvalue=binary_predictions)
    return render_template('prediction.html')
